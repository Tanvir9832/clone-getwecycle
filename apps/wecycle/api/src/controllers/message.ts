...
export const createMessage = async (
  req: IFileRequest<CreateMessageDTO>,
  res,
  next
) => {
  try {
...
export const getMessages = async (
  req: IAuthRequest<null, { bookingId: string }>,
  res,
  next
) => {
  try {
...
export const getLatestMessages = async (
  req: IAuthRequest<GetLatestMessageDTO, { bookingId: string }>,
  res,
  next
) => {
  try {
    ...
    const messages = await Message.aggregate([
      {
        $match: {
          booking: new Types.ObjectId(bookingId),
          ...(lastMessage
            ? { _id: { $gt: new Types.ObjectId(lastMessage) } }
            : {}),
        },
      },
      {
...
export const getConversations = async (req: IAuthRequest, res, next) => {
  try {
    ...
    const isProvider = checkAccess(userType, [
      UserType.PROVIDER,
